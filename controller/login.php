...
if (isset($_POST['signin'])) {
    ...
    // Query to check the username and password directly (without password hashing)
    $login_query = "
        SELECT
            users.user_id, 
            users.username, 
    ...
    if ($login_query_run) {
        if (mysqli_num_rows($login_query_run) > 0) {
            ...
            // Password is directly compared, no hashing or password_verify()
            $userId = $data['user_id'];
            ...
            if ($accountStatus == 'suspended') {
                ...
                if ($userRole == 'admin') {
                    ...
                    header("Location: ../Custoemr/index.php");
